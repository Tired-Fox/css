...
pub const Parser = struct {
    ...
    fn mapCodepoint(self: *Self, codepoint: ?u21) ?u21 {
        if (codepoint) |cp| {
            return switch (cp) {
                0x0C => 0x0A,
                0x0D => {
    ...
    pub fn next(self: *Self) ?u21 {
        if (self.buffer[0]) |c| {
            ...
            self.buffer[2] = null;
            self.location += @intCast(std.unicode.utf8CodepointSequenceLength(c) catch unreachable);
            return c;
        }

        const n = self.mapCodepoint(self.stream.nextCodepoint());
        if (n) |c| {
            self.location += @intCast(std.unicode.utf8CodepointSequenceLength(c) catch unreachable);
        }
        return n;
    ...
    pub fn nextSlice(self: *Self, buffer: []u8) !?u3 {
        if (self.next()) |cp| {
    ...
    ///
    /// @return Number of bytes written to the buffer.
    pub fn peekSlice(self: *Self, buffer: []u8) !?u3 {
        if (self.peek()) |cp| {
            const size = try std.unicode.utf8CodepointSequenceLength(cp);
            if (size > buffer.len) {
                return error.BufferTooSmall;
